         ...
         .Executes(async () =>
         {
             ...
             foreach (var app in apps)
             {
                 ...
                 argoAppParams = argoAppParams.Concat(configs)
                     ...
                     .ToList();
                 if (!string.IsNullOrEmpty(app.Platform.ImageTag))
                     argoAppParams.Add(new ImageTagHelmParameter(app.Platform.ImageTag));
                 if (!string.IsNullOrEmpty(app.Platform.Tier))
                     argoAppParams.Add(new TierHelmParameter(app.Platform.Tier));
                 if (!string.IsNullOrEmpty(app.Platform.ImageRepository))
                     argoAppParams.Add(new ImageRepositoryHelmParameter(app.Platform.ImageRepository));
                 if (!string.IsNullOrEmpty(app.Storefront.ImageTag))
                     argoAppParams.Add(new StorefrontImageTagHelmParameter(app.Storefront.ImageTag));
                 if (!string.IsNullOrEmpty(app.Storefront.ImageRepository))
                     argoAppParams.Add(new StorefrontImageRepositoryHelmParameter(app.Storefront.ImageRepository));
